...
const canManage = computed(
  () => !!currentSession.value?.isOwner || isAllowed(['audit_finding_categories:manage']),
)
...
// Code renders as a chip, and Colour renders the category badge — all via
// slots. Per-row Edit/Deactivate come from rowActions (null hides the actions
// column for users without manage permission).
const columns = [
  ...
  >
    ...
    <div class="tw:p-4">
      <DataTable
        ...
        filterable
        exportManager
        exportFilename="audit-finding-categories.csv"
        persistKey="lookups:auditFindingCategories"
