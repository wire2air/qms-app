 ...
 */
import { IconClipboardCheck, IconAlertTriangle, IconTool, IconExternalLink } from '@tabler/icons-vue'
  ...
  <div class="tw:flex tw:flex-col tw:gap-6">
    <div class="tw:grid tw:grid-cols-1 tw:lg:grid-cols-3 tw:gap-6">
      ...
      <div class="tw:bg-sidebar tw:rounded-xl tw:border tw:border-divider tw:overflow-hidden">
        <div class="tw:px-5 tw:py-3 tw:border-b tw:border-divider tw:bg-main-hover tw:flex tw:items-center tw:gap-2">
          <IconClipboardCheck :size="18" class="tw:text-secondary" />
          <h3 class="tw:font-bold tw:text-on-main">Open Audits</h3>
          <span class="tw:ml-auto tw:text-sm tw:font-semibold tw:text-secondary">{{ openAudits.length }}</span>
        </div>
        <div class="tw:p-3 tw:flex tw:flex-col tw:gap-1.5">
          ...
          >
            <span class="tw:text-sm tw:font-mono tw:text-secondary tw:shrink-0">{{ a.auditNumber }}</span>
            <span class="tw:text-sm tw:flex-1 tw:min-w-0 tw:truncate">{{ a.displayMeta?.standardName || 'Audit' }}</span>
            <span class="tw:text-[10px] tw:uppercase tw:font-semibold tw:text-secondary">{{ a.statusId }}</span>
          </button>
          <p v-if="!openAudits.length" class="tw:text-xs tw:text-secondary tw:italic tw:py-2">No open audits.</p>
        </div>
      </div>
      ...
      <div class="tw:bg-sidebar tw:rounded-xl tw:border tw:border-divider tw:overflow-hidden">
        <div class="tw:px-5 tw:py-3 tw:border-b tw:border-divider tw:bg-main-hover tw:flex tw:items-center tw:gap-2">
          ...
          <span class="tw:ml-auto tw:text-sm tw:font-semibold tw:text-secondary">{{ openNcs.length }}</span>
        </div>
        <div class="tw:p-3 tw:flex tw:flex-col tw:gap-1.5">
          <button
            ...
            @click="openNc(n.id)"
          >
            <span class="tw:text-sm tw:font-mono tw:text-secondary tw:shrink-0">{{ n.ncNumber }}</span>
            <span class="tw:text-sm tw:flex-1 tw:min-w-0 tw:truncate">{{ n.title }}</span>
            <span class="tw:text-[10px] tw:uppercase tw:font-semibold tw:text-secondary">{{ n.statusId }}</span>
          </button>
          <p v-if="!openNcs.length" class="tw:text-xs tw:text-secondary tw:italic tw:py-2">No open NCs.</p>
          <button
            type="button"
      ...
      <div class="tw:bg-sidebar tw:rounded-xl tw:border tw:border-divider tw:overflow-hidden">
        <div class="tw:px-5 tw:py-3 tw:border-b tw:border-divider tw:bg-main-hover tw:flex tw:items-center tw:gap-2">
          ...
          <span class="tw:ml-auto tw:text-sm tw:font-semibold tw:text-secondary">{{ openCapas.length }}</span>
        </div>
        <div class="tw:p-3 tw:flex tw:flex-col tw:gap-1.5">
          <button
            ...
            @click="openCapa(c.id)"
          >
            <span class="tw:text-sm tw:font-mono tw:text-secondary tw:shrink-0">{{ c.capaNumber }}</span>
            <span class="tw:text-sm tw:flex-1 tw:min-w-0 tw:truncate">{{ c.title }}</span>
            <span class="tw:text-[10px] tw:uppercase tw:font-semibold tw:text-secondary">{{ c.statusId }}</span>
          </button>
          <p v-if="!openCapas.length" class="tw:text-xs tw:text-secondary tw:italic tw:py-2">No open CAPAs.</p>
          <button
            type="button"
            class="tw:mt-1 tw:inline-flex tw:items-center tw:gap-1 tw:text-xs tw:font-medium tw:text-primary tw:hover:underline tw:bg-transparent tw:border-0 tw:cursor-pointer tw:self-start"
            @click="viewAllCapas"
