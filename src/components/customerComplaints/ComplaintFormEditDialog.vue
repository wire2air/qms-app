...
watch(model, (open) => {
  if (open) draft.value = makeDraft(props.form)
})
...
async function handleSave() {
  if (!draft.value.name.trim()) {
    toast.notify({ type: 'negative', message: 'Form name is required' })
    return
  }
  saving.value = true
  try {
    ...
    toast.notify({ type: 'negative', message: e.message || 'Failed to save form' })
  } finally {
    saving.value = false
  }
}
...
<template>
  <BaseDialog
    v-model="model"
    :title="form ? 'Edit Form' : 'New Complaint Form'"
    maxWidth="lg"
  >
