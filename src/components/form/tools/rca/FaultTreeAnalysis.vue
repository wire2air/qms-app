...
<template>
  <div class="tw:flex tw:flex-col tw:gap-4">
    ...
    <div v-if="rootNode" class="tw:border-2 tw:border-primary tw:rounded-lg tw:p-3 tw:text-center">
      <div
        class="tw:text-caption tw:font-semibold tw:text-primary tw:uppercase tw:tracking-wider tw:mb-1"
      >
        Top Event
      </div>
      ...
      <div v-for="node in config.nodes ?? []" :key="node.id">
        ...
        >
          <div class="tw:flex tw:flex-col tw:shrink-0 tw:items-start tw:gap-0.5">
            ...
            <span class="tw:text-micro tw:text-secondary"> Gate: {{ node.gate }} </span>
          </div>
    ...
    >
      <div
        class="tw:text-caption tw:font-semibold tw:text-bad tw:uppercase tw:tracking-wider tw:mb-1"
      >
        Critical Path
      </div>
