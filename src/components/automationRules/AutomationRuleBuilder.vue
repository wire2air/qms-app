  ...
  if (props.ruleId && existing.value) {
    ...
    draft.value = {
      ...
      conditions: Array.isArray(r.conditionTree?.conditions)
        ...
        : [],
      actions: Array.isArray(r.actions) ? r.actions.map((a) => ({ ...a, config: { ...(a.config || {}) } })) : [],
      siteIds: [...(r.siteIds || [])],
...
function addCondition() {
  ...
  draft.value.conditions.push({ field: f?.key || '', operator: operatorsForField(draft.value.objectType, f?.key)[0]?.value || 'is', value: '' })
}
...
<template>
  <BaseDialog v-model="open" :title="ruleId ? 'Edit Automation Rule' : 'New Automation Rule'" maxWidth="2xl">
    <div class="tw:flex tw:flex-col tw:gap-4 tw:p-1">
      <div class="tw:grid tw:grid-cols-2 tw:gap-3">
        <BaseField v-slot="{ id }" label="Rule name" required>
          <BaseTextInput :id="id" v-model="draft.name" placeholder="e.g. Notify QA on critical events" />
        </BaseField>
      ...
      <div class="tw:border tw:border-divider tw:rounded-lg tw:p-3">
        <div class="tw:flex tw:items-center tw:justify-between tw:mb-2">
          <div class="tw:flex tw:items-center tw:gap-2">
            ...
            <select v-model="draft.logic" class="tw:border tw:border-divider tw:rounded tw:px-2 tw:py-1 tw:text-xs">
        ...
        <div v-for="(cond, i) in draft.conditions" :key="i" class="tw:flex tw:items-center tw:gap-2 tw:mb-2">
          <select v-model="cond.field" class="tw:border tw:border-divider tw:rounded tw:px-2 tw:py-1.5 tw:text-sm tw:w-40" @change="onFieldChange(cond)">
            <option v-for="f in fields" :key="f.key" :value="f.key">{{ f.label }}</option>
          </select>
          <select v-model="cond.operator" class="tw:border tw:border-divider tw:rounded tw:px-2 tw:py-1.5 tw:text-sm tw:w-40">
          ...
          <BaseTextInput
            ...
            size="sm"
            class="tw:flex-1"
            :placeholder="LIST_OPERATORS.has(cond.operator) ? 'comma,separated,values' : 'value (code / id / text)'"
          />
          <button class="tw:p-1.5 tw:rounded tw:text-secondary tw:hover:text-red-600" title="Remove" @click="removeCondition(i)">
            <IconTrash :size="15" />
          </button>
        </div>
      </div>
      ...
      <div class="tw:border tw:border-divider tw:rounded-lg tw:p-3">
        <div class="tw:flex tw:items-center tw:justify-between tw:mb-2">
          ...
          <div class="tw:flex tw:items-center tw:gap-2">
            <select v-model="newActionType" class="tw:border tw:border-divider tw:rounded tw:px-2 tw:py-1 tw:text-xs">
              <option value="">Add action…</option>
              <option v-for="a in availableActions" :key="a.value" :value="a.value">{{ a.label }}</option>
            </select>
            <BaseButton variant="outline" size="sm" :disabled="!newActionType" @click="() => { addAction(newActionType); newActionType = '' }">
              <IconPlus :size="14" />
            </BaseButton>
          </div>
        </div>
        <p v-if="!draft.actions.length" class="tw:text-xs tw:text-secondary tw:italic">No actions yet — add at least one.</p>
        <div v-for="(action, i) in draft.actions" :key="i" class="tw:flex tw:items-start tw:gap-2 tw:mb-2 tw:border-b tw:border-divider tw:pb-2">
          <div class="tw:flex-1">
            <div class="tw:text-sm tw:font-medium tw:text-on-main tw:mb-1">
              {{ availableActions.find((a) => a.value === action.type)?.label || action.type }}
            </div>
            <GroupSelectMenu v-if="action.type === 'NOTIFY_GROUP'" v-model="action.config.groupIds" :multiple="true" />
            <UserSelectMenu v-else-if="action.type === 'NOTIFY_USER'" v-model="action.config.userIds" :multiple="true" />
            <BaseTextInput
              v-else-if="action.type === 'NOTIFY_EMAIL'"
              v-model="action.config.emailsRaw"
              size="sm"
              placeholder="email1@x.com, email2@y.com"
              @update:modelValue="(v) => (action.config.emails = String(v || '').split(',').map((s) => s.trim()).filter(Boolean))"
            />
          ...
          <button class="tw:p-1.5 tw:rounded tw:text-secondary tw:hover:text-red-600" title="Remove" @click="removeAction(i)">
            <IconTrash :size="15" />
          </button>
        </div>
      </div>
      ...
      <!-- Scope -->
      <div class="tw:border tw:border-divider tw:rounded-lg tw:p-3">
        <span class="tw:text-xs tw:font-bold tw:uppercase tw:text-secondary">Scope (optional)</span>
        ...
        <div class="tw:grid tw:grid-cols-2 tw:gap-3">
          ...
          <BaseField label="Departments">
            <DepartmentSelectMenu v-model="draft.departmentIds" :multiple="true" :required="false" />
          </BaseField>
        </div>
      </div>
    </div>
