...
const columns = [
  ...
  { name: 'appliesTo', label: 'APPLIES TO', field: (row) => scopeLabel(row), align: 'left' },
  { name: 'specification', label: 'SPECIFICATION', field: (row) => specName(row.specificationId), align: 'left' },
  { name: 'sampling', label: 'SAMPLING PLAN', field: (row) => samplingName(row.samplingPlanId), align: 'left' },
  { name: 'workflow', label: 'WORKFLOW', field: (row) => workflowName(row.workflowVersionId), align: 'left' },
  {
    name: 'active',
    label: 'ACTIVE',
    field: 'active',
