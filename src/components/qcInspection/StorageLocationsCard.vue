...
const rows = useLiveQuery(
  ...
  { models: ['StorageLocation'], initial: [] },
)
const deactivated = useLiveQuery(
  async (db) => (await db.StorageLocation.where('id', undefined, { force: true }).exec()).filter((d) => d.deletedAt),
  { models: ['StorageLocation'], initial: [] },
)
...
const form = ref({ code: '', name: '', description: '', conditions: '', siteId: null, displayOrder: 1000 })
...
function openAdd() {
  ...
  form.value = {
    code: '', name: '', description: '', conditions: '', siteId: null,
...
function openEdit(row) {
  ...
  form.value = {
    code: row.code, name: row.name, description: row.description ?? '', conditions: row.conditions ?? '',
    siteId: row.siteId ?? null, displayOrder: row.displayOrder ?? 1000,
  }
  showDialog.value = true
}
watch(showDialog, (v) => {
...
const createRow = useLiveMutation(async (db, payload) => {
  const row = db.StorageLocation.create(payload)
  await row.save()
  return row
})
...
async function handleDeactivate(row) {
  if (!(await confirm({ title: 'Deactivate', message: `Deactivate "${row.name}"? Existing retain samples keep their stored location.`, okLabel: 'Deactivate', danger: true }))) return
  try {
    await row.delete()
...
<template>
  <div class="tw:rounded-xl tw:border tw:border-divider tw:shadow-sm tw:overflow-hidden tw:bg-sidebar">
    ...
    >
      <template #subtitle>Where QC retain samples are stored. Conditions print on the retain-sample labels. Scoped to this company.</template>
      <template #actions>
    ...
    <div class="tw:p-4">
      <DataTable
        ...
        hidePagination
        searchable
        persistKey="lookups:StorageLocation"
        noDataLabel="No storage locations yet. Add one above."
      >
        ...
        <template #body-cell-conditions="{ row }">
          <span class="tw:text-sm tw:text-secondary">{{ row.conditions || '—' }}</span>
        </template>
    ...
    <BaseDialog v-model="showDialog" :title="editing ? 'Edit Storage Location' : 'Add Storage Location'" maxWidth="md">
        ...
        <div class="tw:flex tw:flex-col tw:gap-3 tw:p-1">
          <BaseField label="Name" required :value="form.name" :rules="[required()]">
            <template #default="field">
              <BaseTextInput v-bind="field" v-model="form.name" placeholder="e.g. Cold Storage" />
            </template>
          </BaseField>
          ...
          <div v-if="!editing">
            ...
            <BaseField :value="form.code" :rules="[required()]">
              <template #default="field">
                <BaseTextInput v-bind="field" v-model="form.code" placeholder="COLD_2_8" :disabled="!codeEditable" @input="codeDirty = true" />
              </template>
            </BaseField>
            <p class="tw:text-caption tw:text-secondary tw:mt-1">SCREAMING_SNAKE_CASE. Cannot be changed later.</p>
          </div>

          <BaseField label="Storage conditions" :value="form.conditions" hint="Printed on retain-sample labels — e.g. 2–8 °C, 25 °C/60% RH.">
            <template #default="field">
              <BaseTextInput v-bind="field" v-model="form.conditions" placeholder="e.g. 2–8 °C" />
            </template>
          </BaseField>

          <BaseField label="Site" :value="form.siteId">
