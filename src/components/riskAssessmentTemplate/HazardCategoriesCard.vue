<script setup>
/**
 * Admin card: manage Hazard Categories (per-tenant lookup).
 *
 * Lives under Risk Assessment Templates → Hazard Categories tab. Reads
 * via the syncEngine (the `hazard_categories` table is replicated).
 * Writes go through REST endpoints in `routes/hazardCategories.js`,
 * gated by the company owner or holders of `hazardCategories:manage`.
 *
 * Mirror of RootCauseCategoriesCard. Each row feeds the Risk Assessment
 * widget's hazard-category picker; the label + colour are denormalized
 * onto risk_assessments rows at finalize time so historical reports
...
// Reset saveError on dialog open so a previous error never bleeds into the next invocation.
...
async function onValidSubmit() {
  ...
  try {
    if (editing.value) {
      await patch(`/v1/services/hazardCategories/${editing.value.id}`, {
      ...
      await post('/v1/services/hazardCategories', {
...
async function handleDeactivate(row) {
  ...
  try {
    await del(`/v1/services/hazardCategories/${row.id}`)
...
async function handleRestore(row) {
  try {
    await post(`/v1/services/hazardCategories/${row.id}/restore`, {})
...
<template>
  ...
  >
    ...
    >
      <div>
        <h2 class="tw:text-lg tw:font-semibold tw:text-on-sidebar">Hazard Categories</h2>
    ...
    >
      You don't have the "Manage Hazard Categories" permission. View only.
    </div>
    ...
    <div class="tw:p-4">
      <DataTable
        ...
        columnManager
        exportManager
        exportFilename="hazard-categories.csv"
        persistKey="lookups:hazardCategories"
      ...
      >
        ...
        <template #body-cell-colour="{ row }">
          <HazardCategoryBadge :category="row" />
        </template>
      </DataTable>
          ...
          <div class="tw:grid tw:grid-cols-1 tw:sm:grid-cols-2 tw:gap-3">
            <div>
              <BaseField label="Colour" :value="form.color">
                <template #default="field">
                  <BaseColorPicker v-bind="field" v-model="form.color" allowNull />
                </template>
              </BaseField>
              <p class="tw:text-caption tw:text-secondary tw:mt-1">
