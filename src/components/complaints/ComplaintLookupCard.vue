...
const props = defineProps({
  ...
  parentModel: { type: String, default: null },
...
const rows = useLiveQueryWithDeps(
  [() => props.model],
  async (db, [model]) =>
    (await db[model].where().exec()).sort((a, b) => (a.displayOrder ?? 0) - (b.displayOrder ?? 0)),
  { models: [props.model], initial: [] },
)
const deactivated = useLiveQueryWithDeps(
  [() => props.model],
  async (db, [model]) =>
    (await db[model].where('id', undefined, { force: true }).exec()).filter((d) => d.deletedAt),
  { models: [props.model], initial: [] },
)
...
const columns = computed(() =>
  [
    ...
    props.parentModel ? { name: 'parent', label: props.parentLabel.toUpperCase(), field: 'parent', align: 'left' } : null,
...
const form = ref({ code: '', name: '', description: '', color: '#64748b', displayOrder: 1000, parentId: null })
...
function openAdd() {
  ...
  form.value = {
    code: '', name: '', description: '', color: '#64748b',
    displayOrder: (rows.value?.length ?? 0) * 100 + 100, parentId: null,
  }
...
function openEdit(row) {
  ...
  form.value = {
    ...
    color: row.color ?? '#64748b', displayOrder: row.displayOrder ?? 1000,
    parentId: props.parentField ? (row[props.parentField] ?? null) : null,
  }
  showDialog.value = true
}
watch(showDialog, (v) => {
...
async function onValidSubmit() {
  ...
  try {
    ...
    if (props.hasColor) base.color = form.value.color || null
    if (props.parentField) base[props.parentField] = form.value.parentId || null
...
// Parent-name resolver for the table cell.
const parents = useLiveQueryWithDeps(
  [() => props.parentModel],
  async (db, [pm]) => (pm ? await db[pm].where().exec() : []),
  { models: props.parentModel ? [props.parentModel] : [], initial: [] },
)
function parentName(row) {
  if (!props.parentField) return '—'
  return parents.value.find((p) => p.id === row[props.parentField])?.name ?? '—'
}
</script>
  ...
  <div class="tw:rounded-xl tw:border tw:border-divider tw:shadow-sm tw:overflow-hidden tw:bg-sidebar">
    ...
    <div class="tw:p-4">
      <DataTable
        ...
        hidePagination
        searchable
        :persistKey="`lookups:${model}`"
        noDataLabel="No entries yet. Add one above."
      >
        <template #body-cell-name="{ row }">
          <div class="tw:flex tw:items-center tw:gap-2">
            <span v-if="hasColor" class="tw:size-3 tw:rounded-full tw:shrink-0" :style="{ backgroundColor: row.color || '#cbd5e1' }" />
        ...
        <template #body-cell-parent="{ row }">
          <span class="tw:text-sm tw:text-secondary">{{ parentName(row) }}</span>
        </template>
    ...
    <BaseDialog v-model="showDialog" :title="editing ? `Edit ${title}` : `Add ${title}`" maxWidth="md">
        ...
        <div class="tw:flex tw:flex-col tw:gap-3 tw:p-1">
          <BaseField label="Name" required :value="form.name" :rules="[required()]">
            <template #default="field">
              <BaseTextInput v-bind="field" v-model="form.name" placeholder="e.g. Distributor" />
            </template>
          </BaseField>
          ...
          <div v-if="!editing">
            ...
            <BaseField :value="form.code" :rules="[required()]">
              <template #default="field">
                <BaseTextInput v-bind="field" v-model="form.code" placeholder="DISTRIBUTOR" :disabled="!codeEditable" @input="codeDirty = true" />
              </template>
            </BaseField>
            <p class="tw:text-caption tw:text-secondary tw:mt-1">SCREAMING_SNAKE_CASE. Cannot be changed later.</p>
          </div>

          <BaseField v-if="parentModel" :label="parentLabel" required :value="form.parentId" :rules="[required()]">
            <template #default="field">
              <ComplaintLookupSelectMenu v-bind="field" v-model="form.parentId" :model="parentModel" required />
            </template>
          </BaseField>
          ...
          <div class="tw:grid tw:grid-cols-1 tw:sm:grid-cols-2 tw:gap-3">
            <BaseField v-if="hasColor" label="Color" :value="form.color">
              <template #default="field">
                <input v-bind="field" v-model="form.color" type="color" class="tw:h-9 tw:w-full tw:rounded tw:border tw:border-divider" />
              </template>
            </BaseField>
