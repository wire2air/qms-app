...
// Set by main.js when an unauthenticated request bounced the user: the path
// they actually wanted. Read once here and posted with the credentials so the
// server can put it in the handoff token.
const returnTo = (() => {
  const raw = new URL(window.location.href).searchParams.get('next')
  // Only same-origin absolute PATHS. A value starting `//` is protocol-relative
  // and would leave the app, so it is dropped here as well as on the server.
  return raw && raw.startsWith('/') && !raw.startsWith('//') ? ref(raw) : ref('')
})()
  ...
  try {
    ...
    const body = isSignup.value
      ...
      : {
          email: email.value,
          password: password.value,
          // Where the user was headed before the session check bounced them
          // (see main.js handleSessionExpired). Rides the handoff token and is
          // re-validated server-side by safeNext — never trusted as given.
          ...(returnTo.value ? { next: returnTo.value } : {}),
        }
